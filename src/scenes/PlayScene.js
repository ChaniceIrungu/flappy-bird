...
class PlayScene extends BaseScene {
  ...
  createBird() {
    this.bird = this.physics.add
      .sprite(20, this.screenHeight / 2, "bird")
      .setFlipX(true)
  ...
  placePipe(uPipe, lPipe) {
    ...
    const pipeVerticalPosition = Phaser.Math.Between(
      0 + 20,
      this.screenHeight - 20 - pipeVerticalDistance
    );
    ...
    const distanceBetweenPipes =
      uPipe.x - uPipe.width * this.scaleRatio - rightMostX;
    ...
    for (let i = 0; i < COINS_TO_RENDER; i++) {
      this.coins
        .get(
          coinX,
          uPipe.y +
            Phaser.Math.Between(-150 * this.scaleRatio, 450 * this.scaleRatio),
          "bomb"
        )
